<!--
INCOMPLETE CAPTURE — the paste this was built from started mid-document.
Missing: the YAML frontmatter (name/description/etc.), the opening
principles, the design-system chooser (Section 2, referenced later as
"Section 2.A"), the palette/typography/motion dial definitions as first
introduced (Section 4, referenced later as "Section 4.1" through "4.11"),
and the start of Section 5 / 5.A (sticky-scroll-stack) — this file picks
up mid-explanation of 5.A's critical points, then continues into 5.B.

To complete this file, run on the source machine:
  head -n 400 ~/.claude/skills/taste/SKILL.md
(adjust the line count until it overlaps with the "Critical points:
`start: "top top"`, `pin: true`, every card except the last is pinned"
line this file starts on) and paste the missing head back in — or just
run `cat ~/.claude/skills/taste/SKILL.md` again and paste the whole
thing if scrollback allows it this time.
-->
